#Frequency of predicted-expected genus pairs, where the predicted genus either matches exactly or is the closest genus to the expected taxonomically.
#Displayed as a heat map, with taxonomy tree overlays.
...
dat <- read.tree(file="genera_tree.txt")
pdata <- read.csv("validation_data_vhdb_archaea.csv") #Archaea data
#pdata <- read.csv("validation_data_vhdb_bacteria.csv") #Bacterial data
...
df <- prepare_match_table(pdata, 1)
df_dist <- add_distance_data(df, lineage_tbl_agg)

all_df_genera <- unique(c(unique(as.character(df_dist$PRED_GENERA)),unique(as.character(df_dist$KNOWN_GENERA))))
mytree <- keep.tip(dat, intersect(all_df_genera, dat$tip))
dmat <- cophenetic.phylo(mytree)
hc <- hclust(dist(dmat))
den <- as.dendrogram(hc)
count_mtrx <- known_pred_freq_mtrx(df_dist, as.character(mytree$tip))
...
hm <- heatmap.2(log(count_mtrx + 1),  
                Rowv=den, Colv=den,
                density.info = "none", 
                trace="none", 
                margins=c(15,15), 
                col=colorpanel(255, low="white", high="red"), 
                xlab = paste("Expected genera, colour=log(n(genomes)+1), mismatch allowed=1\nPPV-Order=", round(ppv,4), ", PPV-Family=", round(ppv1,4), ", PPV-Genus=", round(ppv2,4),  sep=""),
                ylab = "Predicted genera, colour=log(n(genomes)+1)",
                scale="none",
                dendrogram="both", 
                key=TRUE,
                reorderfun=NULL)