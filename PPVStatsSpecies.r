#PPV statistics of CRISPRHost_blast (bacterial virus, archaeal plasmid, and bacterial plasmid)
...
prepare_match_table <- function(data_in, max_mismatch = 20) {
	expand_row <- function(x) {
		lst_pred_species <- strsplit(as.character(x$PRED_SPECIES)[1], ";")[[1]]
		lst_pred_species <- gsub(" ", "_", lst_pred_species)
		
		nn <- length(lst_pred_species)
		
		df <- data.frame(ASSEMBLY = rep(as.character(x$ASSEMBLY)[1], nn), 
		                 KNOWN_SPECIES = rep(as.character(x$KNOWN_SPECIES)[1], nn),
					 	 KNOWN_GENERA = rep(as.character(x$KNOWN_GENERA)[1], nn),
		             	 PRED_SPECIES = as.character(lst_pred_species),
		             	 PRED_GENERA = gsub("(\\S+)_\\S+", "\\1", lst_pred_species), 
	...
	all_viral_genome_gcf <- gsub("(\\S+)\\|\\S+\\|\\S+\\|\\S+", "\\1", data_in$VIRAL_GENOME)
	all_viral_genome_exp_genera <- gsub("\\S+\\|\\S+\\|(\\S+)\\|\\S+", "\\1", data_in$VIRAL_GENOME)
	all_viral_genome_exp_species <- gsub("\\S+\\|\\S+\\|\\S+\\|(\\S+)", "\\1", data_in$VIRAL_GENOME)
	all_viral_genome_exp_genera[! all_viral_genome_exp_genera %in% all_genera] <- "NA"
	all_species_matched <- as.character(data_in$KNOWN_SPECIES)
	nmis <- data_in$SPACER_LENGTH - round(data_in$N_NT_MATCHED_IN_HYBRID / 2)

	df_simple <- data.frame(ASSEMBLY = all_viral_genome_gcf, 
							KNOWN_SPECIES = all_viral_genome_exp_species,
  	...
  	df_simple <- df_simple[df_simple$E_VALUE <= 1,]
  	...
  	nr <- dim(df_simple)[1];
  	df_simple_expanded <- do.call(rbind, lapply(1:nr, FUN = function(x){print(x);expand_row(df_simple[x,])}))
	
    df_simple_expanded <- df_simple_expanded[as.character(df_simple_expanded$PRED_SPECIES) != "NA",]
	df_simple_expanded <- df_simple_expanded[as.character(df_simple_expanded$KNOWN_GENERA) != "NA",]
	
	df_simple_expanded$KNOWN_SPECIES <- as.character(df_simple_expanded$KNOWN_SPECIES)
	df_simple_expanded$PRED_SPECIES <- as.character(df_simple_expanded$PRED_SPECIES)
	
    return(df_simple_expanded)
}
...
add_distance_data <- function(df, lineage_tbl) {
    ...
    df_dist <- merge(df_dist, df_lineage_pred, by = c("PRED_GENERA"), all.x=TRUE)
  	...
  	b1 <- as.numeric(as.character(df$KNOWN_SPECIES) == as.character(df$PRED_SPECIES))
  	b2 <- as.numeric(as.character(df$KNOWN_GENERA) == as.character(df$PRED_GENERA))
  	b3 <- as.numeric(as.character(df$KNOWN_FAMILY) == as.character(df$PRED_FAMILY))
  	b4 <- as.numeric(as.character(df$KNOWN_ORDER) == as.character(df$PRED_ORDER))
  	b1[is.na(b1)] <- 0
  	b2[is.na(b2)] <- 0
  	b3[is.na(b3)] <- 0
  	b4[is.na(b4)] <- 0
  	b5 <- (1 - b1) * b2
  	b6 <- (1 - b1) * (1 - b2) * b3
  	b7 <- (1 - b1) * (1 - b2) * (1 - b3) * b4
  	b8 <- (1 - b1) * (1 - b2) * (1 - b3) * (1 - b4)
  	
  	sort_key <- rep(0, dim(df)[1])
  	sort_key[b5 == 1] <- 8 + df[b5 == 1,]$E_VALUE
  	
  	max_sort_key <- max(sort_key)
  	sort_key[b6 == 1] <- ceiling(max_sort_key) + df[b6 == 1,]$E_VALUE
  	
  	max_sort_key <- max(sort_key)
  	sort_key[b7 == 1] <- ceiling(max_sort_key) + df[b7 == 1,]$E_VALUE
  	
  	max_sort_key <- max(sort_key)
  	sort_key[b8 == 1] <- ceiling(max_sort_key) + df[b8 == 1,]$E_VALUE
...
ppv_stats <- function(data_in, lineage_tbl, max_mismatch, org) {
	...
	n_genus <- sum(df_dist$DIST <= 8)
	n_family <- sum(df_dist$DIST <= 16)
	n_order <- sum(df_dist$DIST <= 32)
	
	ppv <- n_order / n_genome_asgn
	ppv1 <- n_family / n_genome_asgn
	ppv2 <- n_genus / n_genome_asgn
	ppv3 <- n_exact / n_genome_asgn
	
	ppvv <- c(ppv, ppv1, ppv2, ppv3)
	ppv_type <- c("PPV-Order", "PPV-Family", "PPV-Genus", "PPV-Species")
	group <- paste("Max mismatch = ", max_mismatch, sep = "")
	lbl <- paste(n_genome_asgn, "sequences assigned")
	
	df_out <- data.frame(ORGANISM = rep(org, 4),
	                     PPV = ppvv,
	                     PPV_TYPE = ppv_type,
	                     MISMATCH_ALLOWED = rep(max_mismatch, 4),
	                     N_GENOME_ASSIGNED = rep(n_genome_asgn,4),
	                     GROUP = rep(group,4),
	                     LBL = c(lbl, "", "", ""))
}

n_mismatches <- 0:2
orgs <- c("Bacterial Virus", "Archaeal Plasmid", "Bacterial Plasmid")
pdata_paths <- c("validation_data_sp_bacteria.csv", "validation_data_archaea_plasmid.csv", "validation_data_bacteria_plasmid.csv")
